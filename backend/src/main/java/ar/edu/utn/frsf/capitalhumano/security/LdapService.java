...
public class LdapService {
    ...
    // Método para validar si el usuario sigue existiendo en LDAP
    public boolean esUsuarioActivo(String nombreUsuario) {
        try {
            LdapQuery query = LdapQueryBuilder.query().where("uid").is(nombreUsuario);
    ...
    // Método para traer el perfil completo (para el /perfil)
    public Map<String, Object> obtenerPerfilUsuario(String nombreUsuario) {
        try {
            LdapQuery query = LdapQueryBuilder.query().where("uid").is(nombreUsuario);
