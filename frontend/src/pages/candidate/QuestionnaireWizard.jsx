...
export default function QuestionnaireWizard() {
        ...
        const initializeWizard = async () => {
            ...
            try {
                ...
                if (!blockToLoad || !total || !start) {
                    const { data } = await tmApi.post(`/api/cuestionarios/${id}/iniciar`);
    ...
    const loadBlock = async (blockNum) => {
        setLoading(true);
        try {
            const { data } = await tmApi.get(`/api/cuestionarios/${id}/bloques/${blockNum}`);
    ...
    const handleNextBlock = async () => {
        ...
        setLoading(true);
        try {
            await tmApi.post(`/api/cuestionarios/${id}/bloques/${currentBlockNumber}`, payload);
