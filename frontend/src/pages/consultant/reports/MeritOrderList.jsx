...
const MeritOrderList = () => {
    ...
    // --- Carga Inicial ---
    useMountEffect(() => {
        tmApi.get('/api/empresas/seleccion')
    ...
    const loadLazyData = async () => {
        ...
        const params = {
            ...
            ...(filters.companyId && { idEmpresa: filters.companyId }),
            ...(filters.positionName && { nombrePuesto: filters.positionName }),
            ...(filters.code && { codigo: filters.code })
        };

        try {
            const response = await tmApi.get('/api/reportes/orden-merito/puestos', { params });
    ...
    const openModal = async (position) => {
        ...
        try {
            const response = await tmApi.get(`/api/reportes/orden-merito/puestos/${position.id}/evaluaciones`);
