...
const QuestionList = () => {
    ...
    useMountEffect(() => {
        tmApi.get('/api/competencias/seleccion')
    ...
    const loadLazyData = async () => {
        ...
        const params = {
            ...
            ...(filters.competencyId && { idCompetencia: filters.competencyId }),
            ...(filters.factorId && { idFactor: filters.factorId }),
            ...(filters.questionName && { nombrePregunta: filters.questionName })
        };

        try {
            const response = await tmApi.get('/api/preguntas', { params });
    ...
    const handleCompetencyChange = (e) => {
        ...
        if (selectedId) {
            tmApi.get(`/api/factores/seleccion?idCompetencia=${selectedId}`)
